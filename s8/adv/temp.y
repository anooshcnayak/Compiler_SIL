...
statements: statement														{ $$ = $1; }
	...
	;
statement : ID '=' expr ';'		 											{ $$ = combine('=', 2, makename($1), $3); if(check(makename($1)) != check($3)) error1(); }
		  | ID '[' expr ']' '=' expr ';'		 							{ $$ = combine('=', 2, makearray($1, $3), $6); if(check(makename($1)) != check($6) || !check($3)) error1(); }
		  | READ '(' ID ')' ';'  											{ $$ = combine(READ, 1, makename($3)); check(makename($3)); }
		  | READ '(' ID '[' NUM ']' ')' ';'  								{ $$ = combine(READ, 1, makearray($3, makenode($5))); check(makename($3)); }
		  | READ '(' ID '[' ID ']' ')' ';'  								{ $$ = combine(READ, 1, makearray($3, makename($5))); check(makename($3)); if(!check(makename($5))) error1();}
		  | WRITE '(' expr ')' ';' 											{ $$ = combine(WRITE, 1, $3); if(!check($3)) error1();}
		  | IF '(' relexpr ')' THEN statements ENDIF ';' 					{ $$ = combine(IF, 2, $3, $6); if(check($3)) error1();  }
		  | IF '(' relexpr ')' THEN statements ELSE statements ENDIF ';' 	{ $$ = combine(IF, 3, $3, $6, $8); if(check($3)) error1(); }
		  | WHILE '(' relexpr ')' DO statements ENDWHILE ';' 			 	{ $$ = combine(WHILE, 2, $3, $6); if(check($3)) error1(); }
	;
expr : NUM    		 			{ $$ = makenode($1); }
	 | ID  			 			{ $$ = makename($1); }
...
relexpr : TR 					{ $$ = makebool(1); }
	 	...
	 	| relexpr AN relexpr	{ $$ = combine(AN, 2, $1, $3); }
	 	| relexpr ORR relexpr	{ $$ = combine(ORR, 2, $1, $3); }
	;
...
void assign_val2(struct node* p, struct retnode res) {
	...
	if(dummy != NULL) {
		...
		if(dummy->type != res.type) error1();
		struct retnode dum = eval(p->childs[1]);

		if(dum.type != integer) error1();

		if(dum.val < ldummy->size) {
			
			ldummy->ptr[dum.val] = res.val;
...
int check(struct node* p) {
	...
	switch(p->type) {
		...
		case array		:   return (check(p->childs[0]) && check(p->childs[1]));
		case operation:		

			switch(p->oper) {
				case '+': 	return (check(p->childs[0]) && check(p->childs[1]));
				case '-': 	return (check(p->childs[0]) && check(p->childs[1]));			
				case '*': 	return (check(p->childs[0]) && check(p->childs[1]));
				case '<': 	return (check(p->childs[0]) && check(p->childs[1]));
				case '>': 	return (check(p->childs[0]) && check(p->childs[1]));
				case GE: 	return (check(p->childs[0]) && check(p->childs[1]));
				case LE: 	return (check(p->childs[0]) && check(p->childs[1]));
				case EQ: 	return (check(p->childs[0]) && check(p->childs[1]));
				case NE: 	return (check(p->childs[0]) && check(p->childs[1]));
			}
	}
}
		...
		case operation:		
			...
			switch(p->oper) {
				...
				case WRITE:	if(p->childs[0]->type == array) {
								...
								if(eval(p->childs[0]).type != integer) {
									printf("Cannot write a boolean value.\n");
									exit(1);
								}
				...
				case AN: 	if(check(p->childs[0], p->childs[1]))
								return make_ret(eval(p->childs[0]).val != eval(p->childs[1]).val, boolean);
							else error1();							
				case ORR: 	if(check(p->childs[0], p->childs[1]))
								return make_ret(eval(p->childs[0]).val != eval(p->childs[1]).val, boolean);
							else error1();							
			}
	}
